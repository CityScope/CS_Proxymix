...
global {
	...
	init {
		...
		ask room + building_entrance{
			...
			inner_shape <- copy(shape);
			ask places {
				myself.inner_shape <- myself.inner_shape - (square(0.5));
			}
...
species pedestrian_path skills: [pedestrian_road] {
	aspect default {
		draw free_space color: #pink border: #violet;
...
species room {
	...
	aspect default {
		draw inner_shape color: standard_color_per_type[type];
...
species people skills: [escape_pedestrian] {
	...
	float speed <- min(2,gauss(3,0.5)) #km/#h;
	bool arrived;
	aspect default {
		draw circle(0.3) color:#red border: #black at: location + {0,0,0.5};
	}
